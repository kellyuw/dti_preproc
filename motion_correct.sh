...
usage_exit() {
  ...
  Usage:
    ...
    -p <PAR file>: raw Philips image format
...
while getopts k:b:r:M:N:o:i:a:p:t:n:EF OPT
 do
 case "$OPT" in
   "k" ) dti="$OPTARG";;
   "b" ) bval="$OPTARG";;
...
if [ `test_varfile $parfile` -eq 0 ]; then
 error_exit "ERROR: cannot find PAR file"
fi
...
if [ "$method" = "eddy" ]; then
 ...
 python `dirname $0`/make_acqparams.py -d $dti -p $parfile -o $tmpdir/acqparams.txt
